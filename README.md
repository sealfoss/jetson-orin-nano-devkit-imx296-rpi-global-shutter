...
| **External trigger mode** | Custom V4L2 boolean control `trigger_mode` (`v4l2-ctl -c trigger_mode=1`, or the companion element's `trigger-mode` property). Arms the sensor's XTR **fast-trigger** mode at the next stream start — the RPi production register pair `CTRL0B(0x300b)=TRIGEN` + `LOWLAGTRG(0x30ae)=FAST`, written in the RPi driver's exact slot (between standby-exit and MIPI start). While triggered, each hardware pulse produces one frame and the **pulse width sets the exposure**; SHS1/VMAX don't apply. Both registers are also written as 0 on every free-run start so a triggered session can never leak into the next run. Usage guide: [`doc/external-trigger-howto.md`](doc/external-trigger-howto.md). |
...
doc/                         Register reference (typ/pdf), mirrored sources,
                             external-trigger-howto.md (using trigger_mode)
```
